locals {
  ...
  aad_webapp_name              = (var.aad_webapp_name != "" ? var.aad_webapp_name : "ADS GoFast Web Portal (${var.environment_tag}-${substr(module.naming.unique-seed, 0, 4)})")
  aad_functionapp_name         = (var.aad_functionapp_name != "" ? var.aad_functionapp_name : "ADS GoFast Orchestration App (${var.environment_tag}-${substr(module.naming.unique-seed, 0, 4)})")
  ...
  purview_ir_app_reg_name      = (var.purview_ir_app_reg_name != "" ? var.purview_ir_app_reg_name : "ADS GoFast Purview Integration Runtime (${var.environment_tag}-${substr(module.naming.unique-seed, 0, 4)})") 
